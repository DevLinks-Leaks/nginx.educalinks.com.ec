...
{	var id_solicitud = document.getElementById('txt_id_solicitud').value;
	...
	document.getElementById('formulario_continuar_solicitud').innerHTML = '<div align="center" style="height:100%;"><img src="'+ruta_imagenes+'/ajax-loader.gif"/></div>';
...
{   var ruta_imagenes = document.getElementById('ruta_imagenes').value;
	...
	{   if ( xhr.readyState === 4 && xhr.status === 200 )
		...
		{   document.getElementById(div).innerHTML = "Hubo un error con su solicitud al sistema. Por favor intente de nuevo.";
			document.getElementById(div).style.display = 'block';
		}
	};
	xhr.send(data);
...
{	var ruta_imagenes = document.getElementById('ruta_imagenes').value;
	document.getElementById('div_formulario_solicitud').innerHTML = '<div align="center" style="height:100%;"><img src="'+ruta_imagenes+'/ajax-loader.gif"/></div>';
	var data = new FormData();
...
{   var data = new FormData();
    ...
    {   if (xhr.readyState==4 && xhr.status==200)
        {   //alert(xhr.responseText);
			...
			if( alright )
			{	alert("Sus datos fueron guardaros correctamente. Por favor, anote el código de su solicitud para poder entrar posteriormente al sistema a consultar sus datos. SU CÓDIGO DE SOLICITUD ES: " + obj['ID_SOLICITUD']);
			}else
			{   alert("Por favor, revisar su solicitud. No todos sus datos fueron ingresados correctamente. (Código de solicitud: " + obj['ID_SOLICITUD'] + ")" );
			}
        }
    };
    xhr.send(data);
