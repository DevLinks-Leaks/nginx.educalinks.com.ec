...
switch($opc){
	case 'post_add':
		...
		$params_form = array($texto,
							 $_POST['curs_para_codi'],
		...
		$sql_form="{call wall_curs_para_prof_add(?,?,?,?,?,?,?)}";
		$stmp_form = sqlsrv_query($conn, $sql_form, $params_form);
		
		//Para auditoría
		$detalle="Curso paralelo código: ".$_POST['curs_para_codi'];
		$detalle.=" Comentario:".$texto;
		registrar_auditoria (39, $detalle);
		...
		$params_post = array($_POST['curs_para_codi']);
	...
	case 'post_main_add':
		...
		$params_post = array($_POST['curs_para_mate_prof_codi']);
