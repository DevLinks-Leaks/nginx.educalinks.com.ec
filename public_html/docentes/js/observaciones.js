...
{
	if (document.getElementById('obs_deta').value.length > 4 )
	{   document.getElementById(div).innerHTML='<div align="center" style="height:100%;"><img src="../imagenes/ajax-loader.gif"/></div>';
		...
		xhr.onreadystatechange=function(){
			if (xhr.readyState==4 && xhr.status==200){
				...
				if (json.state=="success"){				
					...
					$.growl.notice({ title: "Educalinks informa",message: 'Observación agregada con éxito. '+json.result ,duration: 6400});
					$('#btn_obs_add').button('reset');
					$('#modal_new_obse').modal('hide');
					obs_view('div_obs_list',url,alum_curs_para_codi);
				}else{
		...
		$.growl.error({ title: "Educalinks informa",message: 'La observación no puede tener menos de 4 caracteres.' });
