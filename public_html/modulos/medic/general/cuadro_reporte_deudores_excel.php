<?php
	...
	$ancho_para_firmas=20;//se dividirá por el numero de subcolumnas.
	$cont_retirados=0;
	...
	$sql="{call str_consultareptDeudores_busq2 (?,?,?)}";
	...
	{   
		...
		{	if ($columnas[$i][1]=='COMPORTAMIENTO')
			{	genera_detalle($objPHPExcel, $w, 7, substr($columnas[$i][1],0,20), count($subcolumnas),$styleArray);
		...
		{	$asterisco="";
			...
			{	foreach ($subcolumnas as $subcol)
				{	$params = array($columnas[$j][0]);
					...
					{	$w++;
						...
						if ($columnas[$j][2]=='C')
						{	listado($objPHPExcel, $w, $i+9, number_format($nota,2,'.',','), $styleArray, true, false);
						}
						else
						{	if ($nota==0)
            ...
            {	$aux=0;
				...
				{	$params = array($columnas[$j][0]);
					...
					{	$w++;
						...
						if ($columnas[$j][2]=='C')
						{	if ($nota < 7)
							{	listado($objPHPExcel, $w, $i+9, number_format($nota,2,'.',','), $styleArrayRojo, true, false);
							}else
							{	listado($objPHPExcel, $w, $i+9, number_format($nota,2,'.',','), $styleArray, true, false);
							}
			...
			{	$w++;
				if ($notaRP < 7)
				{	listado($objPHPExcel, $w, $i+9, number_format($notaRP,2,'.',','), $styleArrayRojo, true, false);
				}else
				{	listado($objPHPExcel, $w, $i+9, number_format($notaRP,2,'.',','), $styleArray, true, false);
				}
			}
		}
		///////////////////////////////
		...
		{	foreach ($subcolumnas as $subcol)
			{	//Datos de la materia
				...
				{	$w++;
					...
					if ($columnas[$j][2]=='C')
					{	if ($prom_global >= 8)
						{	listado($objPHPExcel, $w, $i+9, number_format($prom_global,2,'.',','), $styleArrayBuenaNota, true, true);
						}else
						{	listado($objPHPExcel, $w, $i+9, number_format($prom_global,2,'.',','), $styleArray, true, true);
						}
		...
		{	$w++;
			if ($notaRP > 8)
			{	listado($objPHPExcel, $w, $i+9, number_format($notaRP,2,'.',','), $styleArrayBuenaNota, true, true);
			}else
			{	listado($objPHPExcel, $w, $i+9, number_format($notaRP,2,'.',','), $styleArray, true, true);
			}
			listado_rotar_texto_celda($objPHPExcel,$w, $i+9,90);
		}
		...
		{	//Fecha
			listado($objPHPExcel, 1, ($i+10), get_fecha_ciudad($current_language,'true','true'), $styleArrayFont, false, true);
		}
		else
		{	//Fecha
		...
		if($print_user=='true')
		{
			listado($objPHPExcel, 1, ($i+10), PrimeraMayuscula(show_this(10000001)) . ': ' . $usuario, $styleArrayFont, false, true);
		}
