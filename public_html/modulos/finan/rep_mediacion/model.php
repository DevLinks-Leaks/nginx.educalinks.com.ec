...
class Rep_mediacion extends DBAbstractModel
{   #propiedades
    public $banc_nombre;
	...
	{	$this->parametros = array( $curs_codi, $niveEcon_codi, $peri_codi, $fechvenc_ini, $fechvenc_fin, $quienes, $prod_codigo );
        $this->sp = "str_consultareptMediacion_busq";
	...
	public function get_header_mediacion( $peri_codi = -1, $producto = '' )
	{	if( ( $peri_codi == NULL ) || ( $peri_codi == "" ) ) 
			$peri_codi = -1;
		$this->parametros = array( $peri_codi, $producto );
        $this->sp = "str_consultareptMediacion_headers";
        $this->executeSPConsulta();
        if (count($this->rows)<=0)
