...
class docAdmin extends DBAbstractModel{
	...
	public function subir_persona_sintesis( $soli_codi, $soli_sint_codi, $soli_sist_docu_nom_file, $soli_sint_docu_desc = "", $usuario_o_ip_forwarded, $ip_remote )
	{   $this->parametros = array( $soli_codi, $soli_sint_codi, $soli_sist_docu_nom_file, $soli_sint_docu_desc, $usuario_o_ip_forwarded, $ip_remote );
		$this->sp = "str_admisiones_sintesis_upload";
		$this->executeSPConsulta();
		if(count($this->rows)>0)
		{   $this->mensaje="¡Exito! Sintesis subida(s) exitosamente.";
		}
		else
		{   $this->mensaje="¡Error! No se pudo hacer la carga.";
		}
		return $this;
    }
