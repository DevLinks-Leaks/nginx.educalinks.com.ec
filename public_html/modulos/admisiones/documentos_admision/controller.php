			...
			{	$lista= "<b>Lista de archivos adjuntos de síntesis académica</b><br><br>".
				...
				{   if( !empty( $row ) )
					{   $quitar= "<td width='10%'><span style='color:red;cursor:pointer;' class='fa fa-times' id='quitar_".$row['soli_sint_codi']."' ".
									...
									" title='Quitar s&iacute;ntesis' onclick=\"js_docAdmin_sintesis_borrar( '".$row['soli_codi']."', '".$row['soli_sint_codi']."','modal_subir_archivo_bandeja', '".$diccionario['rutas_head']['ruta_html_admisiones']."/documentos_admision/controller.php');\"></span></td>";
		...
		{   $cc +=1;
			...
			<tr>
				...
				<td>';
					$construct_table.='
						<input class="option"
								onClick="js_docAdmin_event(\'div_documentos_tabla\',\''.$diccionario['rutas_head']['ruta_html_admisiones'].'/documentos_admision/controller.php\',\'check\', 0,'.
				...
				$construct_table.=
					'<span onClick="js_docAdmin_event_del(\'div_documentos_tabla\',\''.$diccionario['rutas_head']['ruta_html_admisiones'].'/documentos_admision/controller.php\', \'delete\',0,0,'.$row_docu_busq["docu_codi"].',0,0);"
		...
		{   $validado = $subido = $validador = $hidden = ""; $panel_line = "primary";
			...
			{	$subido.= '<li><a href="' . $directorio_sql . $documento_subido . '" target="_blank">Ver archivo subido.</a></li>';
				...
				{   $validador.= '<li><div style="color:black;">Marcar como documento válido '.
									...
									$aux3[$c][0].', '.$per_codi.',\''.$deband.'\',this.checked,\'div_documentos_admisiones\',\''.$diccionario['rutas_head']['ruta_html_admisiones'].'/documentos_admision/controller.php\');"
								...
								<table width="100%">
									...
									<tr><td>
											<button type="button" style="'.$hidden.'" id="btn_subir_documento_'.$aux3[$c][0].'" name="btn_subir_documento_'.$aux3[$c][0].'" 
												class="btn btn-primary" onclick="js_docAdmin_subir_documentos('.$aux3[$c][0].','.$per_codi.','.$peri_codi.',\''.$deband.'\',\'div_documentos_admisiones\',\''.$diccionario['rutas_head']['ruta_html_admisiones'].'/documentos_admision/controller.php\');">'.
