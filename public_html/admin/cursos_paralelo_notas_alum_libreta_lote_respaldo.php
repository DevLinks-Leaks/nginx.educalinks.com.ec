...
<?php 		
	...
	while ($alumno = sqlsrv_fetch_array($alumnos_view))
	{
		...
		$CC_COLUM_index=0;
		  ...
		  $file_exi='../fotos/' . $_SESSION['directorio'] . '/alumnos/'. $peri_codi . '/' . $alumno["alum_codi"] . '.jpg';
		  if (file_exists($file_exi)) {
			$pp=$file_exi;
		  } else {
			$pp='../fotos/alumnos/0.jpg';
		  }
?>
<page>
            ...
            >
			 ...
			 { 
			 	$prom_cc[$CC_COLUM_index] =  $prom_cc[$CC_COLUM_index] + 1; 
              	$prom[$CC_COLUM_index] =  $prom[$CC_COLUM_index] + trunc_round($row_alum_nota_peri_dist_view[$CC_COLUM_index + 10]);
            }?>
          </td>
