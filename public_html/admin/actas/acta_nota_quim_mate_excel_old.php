		...
		$cont_alumnos=0;
	 	unset($notas_prom);
			...
			if ($j!=3) $promedio_parcial=$promedio_parcial+$nota;
						...
						$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow($w+1, $y, ($row_curs_peri_info['nota_refe_cab_tipo']=='C'?number_format($nota,2,'.',','):'-'));
			...
			$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w).($y))->applyFromArray($styleArrayC);
                        ...
                        {
                            $w+=2;
							$promedio_parcial_total=$promedio_parcial/3;
						$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow($w, ($i+9), notas_prom_quali($_SESSION['peri_codi'],$row_curs_peri_info['nota_refe_cab_tipo'],$promedio_parcial_total));
						    $objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow($w+1, ($i+9), ($row_curs_peri_info['nota_refe_cab_tipo']=='C'?number_format($promedio_parcial_total,2,'.',','):'-'));
						...
						$objPHPExcel->getActiveSheet()->getStyle(PHPExcel_Cell::stringFromColumnIndex($w+1).($i+9))->applyFromArray($styleArrayNPromedio);
                            $w+=2;
							$porcentaje_parcial=$promedio_parcial_total*PORC_PARCIAL;
							$y=$i+9;
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow($w, $y, ($row_curs_peri_info['nota_refe_cab_tipo']=='C'?round($porcentaje_parcial,2):'-'));
                        ...
                        {
                            $porcentaje_examen=$nota*PORC_EXAMEN;
                        }
                    }
					//El promedio del curso solo se presenta si es una materia cuantitativa
					if ($row_curs_peri_info['nota_refe_cab_tipo']=='C')
					{
						...
						$x=$w+1;
						$y=$i+9;
						$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow($w, $y, (number_format($porcentaje_examen,2,'.',',')));
						...
						//Nota Quimestre
						$nota_final=round($porcentaje_examen,2)+round($porcentaje_parcial,2);//promedio parcial
						$w+=1;
						$y=$i+9;
