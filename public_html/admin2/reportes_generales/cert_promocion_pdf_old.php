<?
	...
	foreach ($arrMatricula as $valMatricula)
	{
			...
			if ($valMatricula==$arrDatos[$i]['matricula'])
			{
				...
				if (($arrDatos[$i]['mate_tipo']<>"D") and ($arrDatos[$i]['mate_tipo']<>"P"))
				{
					...
					else
					{
						...
						$calificaciones.='<td width="30%" class="enletras"> '.notas_prom_quali_deta($_SESSION['peri_codi'],$arrDatos[$i]['mate_tipo'],$arrDatos[$i]['materia_calificacion']).'</td>';
						$calificaciones.='</tr>';
					}
				}
				else
		...
		$calificaciones.='<td width="10%" class="calificaciones"> <b>'.notas_prom_quali($_SESSION['peri_codi'],$tipo_mate_comp,$nota_comportamiento).'</b></td>';
		$calificaciones.='<td width="30%" class="enletras"> <b>'.notas_prom_quali_deta($_SESSION['peri_codi'],$tipo_mate_comp,$nota_comportamiento).'</b></td>';
		$calificaciones.='</tr>';
		...
		if ($nota_proyecto<>'')
		{
			...
			$calificaciones.='<td width="60%" class="materias" style="border-bottom: none !important;"> <b>PROYECTO</b></td>';
			$calificaciones.='<td width="10%" class="calificaciones"> <b>'.notas_prom_quali($_SESSION['peri_codi'],$tipo_mate_proy,$nota_proyecto).'</b></td>';
			$calificaciones.='<td width="30%" class="enletras"> <b>'.notas_prom_quali_deta($_SESSION['peri_codi'],$tipo_mate_proy,$nota_proyecto).'</b></td>';
		...
		<table width="650" border="0">
			<tr>
				...
				<td class="titulo" width="80%">{$antes_del_nombre} <br/>{$nombre_colegio}</td>
		...
		<table>
			...
			<tr>
				<td colspan="2" class="texto">
					...
					<p>Para certificar suscriben en unidad de acto el(a) {$etiqueta_rector} con el(a) {$etiqueta_secretario} quien certifica.</p>
				</td>
			</tr>
			<tr>
				<td colspan="2"><br /></td>
