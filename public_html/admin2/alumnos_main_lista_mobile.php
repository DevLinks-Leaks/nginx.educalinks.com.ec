...
<table class="table table-striped" id="alum_table">
 <thead style='background-color:rgba(1, 126, 186, 0.1) !important;'>
  <tr>
    <th width="100%" class="sort"><span class="icon-sort icon"></span>Alumno</th>
  </tr>
 </thead>
 ...
 <?php  
	while ($row_alum_busq = sqlsrv_fetch_array($alum_busq)) 
	{
		...
		$cc +=1; ?>
		<tr>
			<td><b><?php echo $nombre_completo; ?></b><br>
				<small><b>Código:</b> <?php echo $row_alum_busq["alum_codi"]; ?></small><br>
				<small><b>Estado:</b> <?php echo $row_alum_busq["esta_deta"]; ?></small> | <small><b>Curso:</b> <?php echo $row_alum_busq["curs_deta"]." - ".$row_alum_busq["para_deta"]; ?></small><br>
				<br>
				<?php 
				unset($opciones);
				$tabla = "";
				...
				$tabla=alumnos_main_genera_tabla_por_columnas($opciones, 2, 0,'100%','left');// función está en funciones.php
				echo "<div id='div_".$cc."'  class='alumnos_opciones collapse'><hr>".$tabla."</div>";
				
			?>
				<p style='text-align:center;'><button type='button' class='btn btn-default' onclick='js_alumnos_show_options("div_<?php echo $cc; ?>")'><i class=' fa fa-list'></i> Opciones</button></p>
			</td>
		</tr>
	<?php  
