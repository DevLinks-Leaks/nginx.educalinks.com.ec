...
{	private $codigo;
	...
	{	$logo_cole = $_SESSION['ruta_foto_logo_libreta'];
		$this->Image($logo_cole, 5, 10, 20, 18, 'PNG', '', 'C', false, 300, '', false, false, 0, false, false, false);
		$this->SetFont('helvetica', 'B', 8);
		$this->MultiCell(70, 10, '', 0, 'C', 0, 1, '', '', true);
		$this->MultiCell(70, 5, 'BOLETÍN DE CALIFICACIONES', 0, 'L', 0, 1, 25, '', true);
		$this->MultiCell(0, 5, mb_strtoupper($this->periodo,'utf8'), 0, 'L', 0, 1, 25, '', true);
		$this->MultiCell(0, 5, mb_strtoupper($this->curso,'utf8'), 0, 'L', 0, 1, 25, '', true);
		$this->MultiCell(0, 5, 'AÑO LECTIVO '.$_SESSION['peri_deta'], 0, 'L', 0, 1, 25, '', true);
		$this->MultiCell(0, 0, 'ESTUDIANTE:', 0, 'L', 0, 1, 135, 13, true);
		$this->SetFont('helvetica', '', 8);
		$this->MultiCell(0, 0, mb_strtoupper(substr($this->nombre.' '.$this->apellido,0,45),'utf8'), 0, 'L', 0, 1, 135, 17, true);
		$this->MultiCell(0, 0, $this->codigo, 0, 'L', 0, 1, 135, 22, true);
	}
	...
	{	if ($alumno["totalPendiente"]==0 && $alumno["prod_nombre"]==NULL)
		{
			...
			{	$params = array('P', $_SESSION['peri_codi']);
				...
				while ($row_nota_peri_cual_tipo_view = sqlsrv_fetch_array($nota_peri_cual_tipo_view))
				{	$tabla_proyecto.='<tr>';
					$tabla_proyecto.='<td class="tabla_informativa"> ('.$row_nota_peri_cual_tipo_view['nota_peri_cual_refe'].') '.$row_nota_peri_cual_tipo_view['nota_peri_cual_deta'].'</td>';
					$tabla_proyecto.='</tr>';
				}
			...
			$tabla_observaciones.='<tr>';
			$tabla_observaciones.='<td class="tabla_informativa" height="47" rowspan="4"> '.$observaciones['nota_obse_deta'].'</td>';
			$tabla_observaciones.='</tr>';
			$tabla_observaciones.='</table>';
			/*Firmas*/
			$tabla_firmas = '<table>';
			$tabla_firmas.= '<tr>';
			$tabla_firmas.= '<td class="firmas centrar">____________________________<br/>Tutor(a)</td>';
			$tabla_firmas.= '<td class="firmas centrar">____________________________<br/>Representante</td>';
			$tabla_firmas.= '</tr>';
			$tabla_firmas.= '</table>';
			/*Contraseñas*/
			if (para_sist(7))
			{	$tabla_contraseñas = '<table width="100%" border="1" cellpadding="1" cellspacing="0">';
				...
				$tabla_contraseñas.= '</tr>';
				$tabla_contraseñas.='<tr>';
				$tabla_contraseñas.='<td class="tabla_informativa"> <b>Usuario:</b> '.$row_alum_info['alum_usua'].' - <b>Clave:</b> '.$row_alum_info['alum_pass'].'</td>';
				$tabla_contraseñas.='<td class="tabla_informativa"> <b>Usuario:</b> '.$repr_info['usuario'].' - <b>Clave:</b> '.$repr_info['clave'].'</td>';
				$tabla_contraseñas.='</tr>';
...
<table width="99%">
<tr>
<td width="40%">{$tabla_comportamiento}</td>
<td width="50%"> {$tabla_cuantitativa}</td>
