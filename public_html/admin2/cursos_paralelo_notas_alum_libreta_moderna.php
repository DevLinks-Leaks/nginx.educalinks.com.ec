...
<div  class="libreta" >
      <div class="header_institution">
        ...
        <div class="institution">
          ...
          <div class="name">
                  <h4 style="margin: 2px 0;"><strong> <?= para_sist(36).' '.para_sist(3); ?> </strong></h4>
      ...
      <table class="table_striped">
        <thead>
          <tr>
			...
			{   echo '<th style="text-align: center">'.$row_peri_dist_padr_view['peri_dist_abre'].'</th>';
		...
		{ 	$cc +=1; ?> 
			  <tr>
				...
				<td width="34%" >
				  <div  <?php if ($row_alum_nota_peri_dist_view["mate_padr"] >0) echo 'style="text-indent:15px;"';?>  >
					<?php echo $row_alum_nota_peri_dist_view['mate_deta']; ?>
				  </div>
				</td>
				<? $CC_COLUM_index =0; while( $CC_COLUM_index <= $CC_COLUM )  {?>       
				<td align="center"
				<?php
					/*if( $cabecera[$cc]['abre'] == 'VW' ) //DE AHORA EN ADELANTE LAS 'VW' SOLO SERÁN PARA PORCENTAJES.
					{   //--EDUCALINKS PENDIENTE-- parametrizar cual es la nota minima aceptable.
					}
					else
					{   $mayor_aceptable = 7; //--EDUCALINKS PENDIENTE-- parametrizar cual es la nota minima aceptable.
					}*/
				...
				>
					<? if ($row_alum_nota_peri_dist_view['mate_tipo'] <> 'D' and $row_alum_nota_peri_dist_view['mate_tipo'] <> 'P') {?> 
						<?=
						(truncar($row_alum_nota_peri_dist_view[$CC_COLUM_index + 12])==0)?'':truncar($row_alum_nota_peri_dist_view[$CC_COLUM_index + 12]);
						 ?>                      
					<? } ?>
	...
	<div style="border: 1px solid #999; height: 25px; margin-top: 5px;">
    	<strong>Por lo tanto ha sido promovido(a) al <?php echo $row_curs_info["curs_deta_next"];?> </strong>
  	</div>
	<?}?>
  </div>  
     <?php
 ...
 <div class="CSSTableGenerator half_end" >
  ...
  <table   class="table_striped"   >
        ...
        <tbody>
          ...
          <tr>
            <td ><?= $row_nota_peri_cual_tipo_view['nota_peri_cual_deta']; ?></td>
            <td ><?= truncar($row_nota_peri_cual_tipo_view['nota_peri_cual_ini']); ?> - 
			<?= truncar($row_nota_peri_cual_tipo_view['nota_peri_cual_fin']); ?></td>
